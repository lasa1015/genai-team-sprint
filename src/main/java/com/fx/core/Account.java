...
public class Account {
    ...
    public void withdraw(double amount) throws InsufficientFundsException {
        // TODO Task 1 (exceptions recap): when amount > balance, THROW
        //   new InsufficientFundsException(amount, balance)   — instead of just warning.
        // The frozen message must name BOTH numbers (requested AND available) — Task 2
        // writes a test that asserts exactly that. (The `throws` clause is already here.)
        if (amount > balance) { System.out.println("WARN: insufficient funds — withdrawal refused"); return; }
        balance -= amount;
    }
